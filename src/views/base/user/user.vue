...
<div>
  <fieldset ref="fieldset">
    ...
    <el-form inline ref="searchForm" :model="searchInfo">
      ...
      <el-form-item label="性别" prop="gender">
        <el-select clearable filterable class="filter-item" v-model="searchInfo.gender" style="width: 160px">
          <el-option label="全部" value=""></el-option>
          <el-option :label="item.text" :value="item.code" v-for="(item, index) in genderList" :key="index"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="状态" prop="status">
        <el-select filterable clearable class="filter-item" v-model="searchInfo.status" style="width: 100%">
          <el-option label="全部" value=""></el-option>
  ...
  <div style="margin-top: 20px; border-top: 1px solid #ddd">
    ...
    <el-table-pagination ref="table" :columns="columns" :data="tableData" :showPagination="showPagination" highlightCurrentRow :height="450" :page-sizes="[20, 50, 100]" :total="total" :showDetailData=false @current-change="currentChange"
      ...
      <el-table-column slot="append" fixed="right" label="操作" width="140" align="center">
        <template slot-scope="scope">
          <el-button type="text" @click="handleEnable(scope.row)" :class="{'disable': scope.row.status === 0}">{{scope.row.status === 1 ? '禁用': '启用'}}</el-button>
  ...
  <edit-user v-if="addOrEditVisible" ref="addOrEdit" @refreshDataList="getList" :genderList="genderList" :statusList="statusList"></edit-user>
</div>
</template>
...
export default {
  ...
  methods: {
    ...
    async getList() {
      ...
      let res = await API.user.list(`name=${formData.name}&tel=${formData.tel}&email=${formData.email}&status=${formData.status}&gender=${formData.gender}&page=${formData.page}&rows=${formData.rows}`);
      if (res.data) {
    ...
    async removeReq(key) {
      let res = await API.typeOfQualification.remove(key);
      if (res.success) {
        this.$notify({
          title: "成功",
          message: "操作成功",
